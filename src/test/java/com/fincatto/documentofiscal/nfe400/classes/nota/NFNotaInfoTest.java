...
public class NFNotaInfoTest {
    ...
    public void deveGerarXMLDeAcordoComOPadraoEstabelecido() {
		...
		List<?> processos =
			    nfNotaInfoIntermediador
			        .getInformacoesAdicionais()
			        .getProcessosRefenciado();

			if (processos != null && !processos.isEmpty()) {
			    Object first = processos.get(0);
			    if (first != null) {
			        ((NFNotaInfoProcessoReferenciado) first)
			            .setTipoAtoConcessorio(
			                NFTipoAtoConcessorio.TERMO_DE_ACORDO
			            );
			    }
			}
